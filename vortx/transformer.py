...
class MlpBlock(torch.nn.Module):
    ...
    def __init__(self, in_dim, mlp_dim, out_dim, dropout_rate=0.0):
        ...
        # # 自适应卷积
        # # 根据输入通道数自适应调整卷积核大小
        # kernel_size = int(abs((math.log(mlp_dim, 2) + 1) / 2))
        # # 如果卷积核大小是奇数，就使用它
        # if kernel_size % 2:
        ...
        # self.fc11 = nn.Conv1d(in_channels=1, out_channels=1, kernel_size=kernel_size,
        ...
        # torch.nn.init.kaiming_normal_(self.fc11.weight)##############
        # torch.nn.init.zeros_(self.fc11.bias)##########################
    ...
    def forward(self, x):
        ...
        # out = out.view([20, 1, -1])
        # # x_i = x_i.view([x_i.shape[0], 1, -1])
        # out = self.fc11(out)###加一层自适应#########################
        # out = out.view([20, x.shape[1], -1])
